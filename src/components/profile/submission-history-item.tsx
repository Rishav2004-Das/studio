...
export function SubmissionHistoryItem({ submission, task }: SubmissionHistoryItemProps) {
  return (
    <Card className="overflow-hidden shadow-md transition-shadow hover:shadow-lg">
      <CardContent className="p-4">
        <div className="flex flex-col gap-4 sm:flex-row">
          {submission.fileUrl && (submission.fileUrl.startsWith('https://picsum.photos') || submission.fileUrl.match(/\.(jpeg|jpg|gif|png)$/)) && (
      ...
      <CardFooter className="flex items-center justify-between bg-muted/30 p-3">
        ...
        {submission.status === "Approved" && submission.tokensAwarded && (
